...
<div class="formbody">
    ...
    <form action="<%= basePath%>/biz/SysUser_update.action" method="post" id="commonform">
        ...
        <ul class="forminfo">
            <li><label>用户编号</label><input name="sysUser.userId" type="text" readonly="ture" id="userId" class="dfinput"
                                          value="${sysUser.userId}" userId="${sysUser.userId}"/><i><font
                    color="#FF0000">*用户编号不可更改</font></i><span id="gradeInfo"></span></li>
            <li><label>昵称</label><input name="sysUser.userName" type="text" class="dfinput"
                                        value="${sysUser.userName}"/><i><font color="#FF0000">*必填</font>不能超过20个汉字</i>
            </li>
            <li><label>真实姓名</label><input name="sysUser.userRealName" type="text" class="dfinput"
                                          value="${sysUser.userRealName}"/><i><font color="#FF0000">*必填</font>不能超过20个汉字</i>
            </li>
            <li><label>新密码</label><input id="newPassword" name="newPassword" type="password" class="dfinput"/><i><font
                    color="#FF0000">(若不修改可不填)</font>6-16位字母、数字或下滑线组成</i></li>
            <li><label>确认密码</label><input id="againPassword" name="sysUser.userPassword" type="password"
                                          class="dfinput"/><span id="againPasswd"></span></li>
            <!-- <li><label>头像</label><input id="userImg" name="sysUser.userImg" type="text"  class="dfinput" value="${sysUser.userImg}"/></li> -->
            <li><label>联系方式</label><input name="sysUser.userTelephone" type="text" class="dfinput"
                                          value="${sysUser.userTelephone}"/></li>
            <li><label>邮箱</label><input name="sysUser.userEmail" type="text" class="dfinput"
                                        value="${sysUser.userEmail}"></input></li>
            <li><label>QQ</label><input name="sysUser.userQq" type="text" class="dfinput"
                                        value="${sysUser.userQq}"></input></li>
            <li><label>地址</label><input name="sysUser.userAddress" type="text" class="dfinput"
                                        value="${sysUser.userAddress}"/></li>


            <input name="sysUser.merchantId" id="merchantId" type="hidden" value="${sysMerchantUnit.merchantId }"/>
            <li><label>所属单位</label><input name="name" type="text" class="dfinput" id="merchant"
                                          value="${sysMerchantUnit.merchantName }" onkeyup="find()"></input>
                <div id="items" style="top:615px"></div>
            </li>
                    ...
                    <select id="provinceId" name="tourProductProvince" class="select2">
                        ...
                        <s:iterator value="comDistricts">
                            <c:if test="${name == sysUser.tourProductProvince }">
                                <option value="${name }" id="${id }" selected="selected">${name }</option>
                            </c:if>
                            <c:if test="${name != sysUser.tourProductProvince }">
